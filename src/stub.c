...
void overwrite(unsigned char* ptr, size_t len) {
...
void stub_main(STUB_CONFIG* config) {
    ...
    overwrite((unsigned char*)buffer, config->encrypted_size);
    ...
    for (uint16_t i = 0; i < nt->FileHeader.NumberOfSections; i++) {
        if (sec[i].SizeOfRawData == 0 && sec[i].Misc.VirtualSize > 0) {
            ...
            overwrite(bss, sec[i].Misc.VirtualSize);
