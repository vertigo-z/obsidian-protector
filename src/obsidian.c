...
int pack_pe(uint8_t** pe_data, size_t* pe_size, uint8_t* stub, size_t stub_size) {
    ...
    if (g_pyinstaller_build) {
        IMAGE_SECTION_HEADER* secs_tmp = IMAGE_FIRST_SECTION(nt);
        uint32_t pe_raw_end = 0;
        for (uint16_t i = 0; i < nt->FileHeader.NumberOfSections; i++) {
            uint32_t sec_end = secs_tmp[i].PointerToRawData + secs_tmp[i].SizeOfRawData;
            if (sec_end > pe_raw_end) pe_raw_end = sec_end;
        }
        DBG_HEX("PE raw data ends at", pe_raw_end);

        if (*pe_size > pe_raw_end) {
            overlay_size = (uint32_t)(*pe_size - pe_raw_end);
            overlay = (uint8_t*)malloc(overlay_size);
            if (overlay) {
                memcpy(overlay, *pe_data + pe_raw_end, overlay_size);
                INFO("Extracted PyInstaller overlay: %u bytes at file offset 0x%X", overlay_size, pe_raw_end);
                *pe_size = pe_raw_end;
            }
        }
    }
    ...
    if (overlay && overlay_size > 0) {
        DBG("Encrypting python overlay");
        obfuscate_data(overlay, overlay_size, key);

        WWN_PYI_FOOTER footer = {0};
        memcpy(footer.magic, "WWNPYI1!", 8);
        footer.key = key;
        footer.pkg_offset = *pe_size;
        footer.pkg_size = overlay_size;

        DBG("Encrypted WWN PyInstaller archive, initiating realloc");
        size_t new_size = *pe_size + overlay_size + sizeof(WWN_PYI_FOOTER);
        uint8_t* new_pe = (uint8_t*)realloc(*pe_data, new_size);
        DBG("Realloc completed");
        if (new_pe) {
            *pe_data = new_pe;
            dos = (IMAGE_DOS_HEADER*)*pe_data;
            nt = (IMAGE_NT_HEADERS*)(*pe_data + dos->e_lfanew);
            stub_sec = IMAGE_FIRST_SECTION(nt) + (nt->FileHeader.NumberOfSections - 1);
            stub_location = *pe_data + stub_sec->PointerToRawData;
            config_location = (STUB_CONFIG*)(stub_location + stub_size);

            memcpy(*pe_data + *pe_size, overlay, overlay_size);
            memcpy(*pe_data + *pe_size + overlay_size, &footer, sizeof(footer));
            config_location->overlay_rva = 0;
            config_location->overlay_size = 0;
            config_location->py = 0;

            *pe_size = new_size;
            SUCCESS("Encrypted WWN PyInstaller archive appended at 0x%llX", (unsigned long long)footer.pkg_offset);
        } else {
            ERR("Failed to realloc for encrypted PyInstaller archive");
            free(overlay);
            return 1;
        }
        free(overlay);
...
void script_stem(const char* path, char* out, size_t out_size) {
    ...
    len = strlen(base);
    if (len > 3 && base[len - 3] == '.' && base[len - 2] == 'p' && base[len - 1] == 'y') {
        len -= 3;
    }
...
int has_ext_ci(const char* path, const char* ext) {
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--pink") == 0) {
            ...
            arg_offset++;
        } else if (strcmp(argv[i], "--pyinstaller") == 0 || strcmp(argv[i], "-pyinstaller") == 0) {
